...
def демо_бары(цена: float, вход: float, стоп: float) -> list[dict]:
    """Правдоподобные бары для учебной карточки. Демо должно выглядеть
    ровно как настоящий сигнал, иначе оно не готовит к настоящему:
    цена подходит к уровню входа сверху и разворачивается."""
    из: list[dict] = []
    шаг = (цена - вход) / 12 if цена > вход else цена * 0.002
    т = цена + шаг * 10
    for i in range(40):
        т = т - шаг * (1.0 if i < 30 else -0.6)
        размах = abs(шаг) * 1.4 or цена * 0.003
        о = т + размах * (0.3 if i % 2 else -0.3)
        из.append({"o": round(о, 2), "h": round(max(о, т) + размах * 0.5, 2),
                   "l": round(min(о, т) - размах * 0.5, 2), "c": round(т, 2)})
    return из
...
def демо_сигнал(цена_btc: float, cfg: dict) -> dict:
    ...
    return {
        ...
        "бары": демо_бары(цена_btc, вход, стоп),
...
def распределение_R() -> dict:
    """Результаты всех закрытых сделок в единицах риска — для гистограммы.

    Форма распределения и есть характер системы: у трендовой она резко
    несимметрична, и семь стопов подряд на такой форме — норма, а не поломка.
    Числом это не объяснить, картинкой видно сразу."""
    значения: list[float] = []
    for файл, ключ, поле in (("breakouts.json", "позиции", "r"),
                             ("paper_long.json", "позиции", "r"),
                             ("shadow.json", "сделки", "результат_R"),
                             ("paper.json", "сделки", "результат_R")):
        п = os.path.join(БАЗА, "state", файл)
        if not os.path.exists(п):
            continue
        try:
            with open(п, encoding="utf-8") as f:
                записи = json.load(f).get(ключ, []) or []
        except (json.JSONDecodeError, OSError):
            continue
        for з in записи:
            if з.get("состояние") not in ("закрыта", "завершена"):
                continue
            if з.get("исход") in ("не дошло", "истёк срок"):
                continue
            r = з.get(поле)
            if r is not None:
                значения.append(float(r))
    if not значения:
        return {"всего": 0, "корзины": []}

    # Корзины по 0.5R от -2 до +4, крайние собирают всё за пределами.
    края = [-99, -1.5, -1.0, -0.5, 0.0, 0.5, 1.0, 1.5, 2.0, 3.0, 99]
    подписи = ["хуже -1.5", "-1.5…-1", "-1…-0.5", "-0.5…0", "0…0.5",
               "0.5…1", "1…1.5", "1.5…2", "2…3", "лучше 3"]
    корзины = [0] * (len(края) - 1)
    for з in значения:
        for i in range(len(края) - 1):
            if края[i] <= з < края[i + 1]:
                корзины[i] += 1
                break
    return {
        "всего": len(значения),
        "корзины": [{"метка": п, "сколько": к, "минус": i < 4}
                    for i, (п, к) in enumerate(zip(подписи, корзины))],
        "средний": round(sum(значения) / len(значения), 3),
        "лучшая": round(max(значения), 2),
        "худшая": round(min(значения), 2),
    }
...
def разобрать_всё(капитал: float, риск_пр: float, плечо: float) -> dict:
    ...
    # Монеты, которые короткая система не рекомендует: сигнал по ним
    # не показывается как «ДА», иначе кабинет советовал бы то, от чего
    # система отказалась по замеру (config.json, сигналы).
    ...
    for актив in cfg["активы"]:
        ...
        if а.вывод == signals.СИГНАЛ and а.план and актив in исключить:
            запись["вывод"] = signals.НЕТ_СИГНАЛА
            запись["почему"] = ("Сигнал есть, но короткая система эту монету "
                                "не торгует: на истории она убыточна в обеих "
                                "половинах. Сигнал ведётся в тени для проверки.")
        elif а.вывод == signals.СИГНАЛ and а.план:
            p = а.план
            ликв = risk.цена_ликвидации(а.направление, p.вход, плечо)
            # Последние бары для мини-графика: вход, стоп и цель тремя
            # числами не говорят, ГДЕ мы входим. На фоне свечей сразу видно,
            # на откате мы или в середине движения.
            запись["бары"] = [
                {"o": свечи.opens[i], "h": свечи.highs[i],
                 "l": свечи.lows[i], "c": свечи.closes[i]}
                for i in range(max(0, len(свечи) - 40), len(свечи))
            ]
            запись["сделка"] = {
                "направление": а.направление,
                "уверенность": а.уверенность,
                "вход": p.вход, "стоп": p.стоп, "цель": p.цели[0],
                # atr нужен журналу решений: по нему ведётся стоп ОСТАТКА
                # после частичной фиксации на цели.
                "atr": p.atr,
    ...
    return {
        ...
        # Данные для графиков. Кабинет был полностью текстовым, и профиль
        # системы — много мелких минусов, редкие крупные плюсы — числами
        # не передаётся. Картинкой он понятен сразу.
        "кривая": equity.за_период(90),
        "кривая_сводка": equity.сводка(),
        "распределение": распределение_R(),
