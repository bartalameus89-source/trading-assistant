...
def ожидание(плечо: float = 1.0) -> dict:
    ...
    try:
        ...
        return пусто
    # Монеты, которые короткая система больше не торгует, в статистику
    # не идут: иначе кабинет обещал бы результат сделок, которых не будет.
    исключить = set(конфиг()["сигналы"].get("исключить_из_короткой", []))
    сделки = [с for с in сделки if с.get("актив") not in исключить]
    if not сделки:
        return пусто
    ...
    # Порог безубыточности — по ФАКТИЧЕСКИМ средним выигрышу и проигрышу:
    # доля побед p, при которой p·выигрыш = (1−p)·проигрыш. Раньше формула
    # считала, что каждая удачная сделка даёт ровно цель, а каждая неудачная
    # теряет полный риск. Но многие неудачные закрываются в ноль по
    # безубытку: средний проигрыш втрое меньше среднего выигрыша, и порог
    # около 25%, а не 45%. Кабинет писал «порог 45%, у нас 36%» — будто мы
    # теряем, — при положительном итоге на сделку.
    ср_выигрыш = (sum(победы) / len(победы)) if победы else 0.0
    ср_проигрыш = abs(sum(проигрыши) / len(проигрыши)) if проигрыши else 0.0
    if ср_выигрыш + ср_проигрыш > 0:
        порог = ср_проигрыш / (ср_выигрыш + ср_проигрыш) * 100
    else:
        цель_R = cfg_["риск"]["рабочая_цель_R"]
        порог = (1 + 0.125) / (1 + цель_R) * 100     # данных нет — по цели

    # Сколько месяцев покрывает выборка — по датам самих сделок. Раньше
    # здесь стояла формула для 1000 баров, а бэктест давно идёт на 6000:
    # 290 сделок делились на 5 месяцев вместо 33, и кабинет писал
    # «100 сделок — примерно месяц торговли» вместо почти года.
    времена = [с["время_входа"] for с in сделки if с.get("время_входа")]
    if len(времена) >= 2:
        секунд = (max(времена) - min(времена)) / (1000 if max(времена) > 1e11 else 1)
        месяцев = max(секунд / 86400 / 30.4, 1.0)
    else:
        месяцев = (1000 - 240) * 4 / 24 / 30.4     # старый файл без дат
    в_месяц = len(R) / месяцев
    ...
    return {"сделок": len(R), "доля_побед": len(победы) / len(R) * 100,
            ...
            "сделок_в_месяц": round(max(в_месяц, 0.1), 1), "порог_побед": порог,
